...
sendVarToJs('jeedomBackgroundImg', 'plugins/easycontrol/core/img/panel.jpg');
$date = array(
...
if (count($object->getEqLogic(true, false, 'easycontrol')) == 0) {
	foreach ($allObject as $object_sel) {
		if (count($object_sel->getEqLogic(true, false, 'easycontrol')) > 0) {
			$object = $object_sel;
			break;
		}
	}
...
<div class="row row-overflow" id="div_easycontrol">
        ...
        <div class="bs-sidebar">
            <ul id="ul_object" class="nav nav-list bs-sidenav">
				...
				foreach ($allObject as $object_li) {
					if ($object_li->getIsVisible() != 1 || count($object_li->getEqLogic(true, false, 'easycontrol', null, true)) == 0) {
						continue;
					}
					$margin = 5 * $object_li->parentNumber();
		if ($object_li->getId() == init('object_id')) {
						echo '<li class="cursor li_object active" ><a data-object_id="' . $object_li->getId() . '" href="index.php?v=d&p=panel&m=easycontrol&object_id=' . $object_li->getId() . '" style="padding: 2px 0px;"><span style="position:relative;left:' . $margin . 'px;">' . $object_li->getHumanName(true,true) . '</span></a></li>';
		} else {
						echo '<li class="cursor li_object" ><a data-object_id="' . $object_li->getId() . '" href="index.php?v=d&p=panel&m=easycontrol&object_id=' . $object_li->getId() . '" style="padding: 2px 0px;"><span style="position:relative;left:' . $margin . 'px;">' . $object_li->getHumanName(true,true) . '</span></a></li>';
		}
	}
				?>
         </ul>
...
<?php include_file('desktop', 'panel', 'js', 'easycontrol');?>
