...
try {
    ...
    if (init('action') == 'resetConso') {
        $eqLogics = eqLogic::byType('easycontrol');
    ...
    if (init('action') == 'getThermostat') {
        ...
        $return['date'] = $date;
        foreach ($object->getEqLogic(true, false, 'easycontrol') as $eqLogic) {
    ...
    if (init('action') == 'getLinkCalendar') {
        ...
        $easycontrol = easycontrol::byId(init('id'));
        if (!is_object($easycontrol)) {
            throw new Exception(__('Equipement easycontrol non trouvé : ', __FILE__) . init('id'));
        }
        try {
        ...
        foreach ($programmablecmds as $name) {
            $thermostat_cmd = $easycontrol->getCmd(null, $name);
        ...
        $thermostat_cmd = $easycontrol->getCmd(null, 'thermostat');
        if (is_object($thermostat_cmd)) {
            foreach (calendar_event::searchByCmd($thermostat_cmd->getId()) as $event) {
                $return[$event->getId()] = $event;
            }
